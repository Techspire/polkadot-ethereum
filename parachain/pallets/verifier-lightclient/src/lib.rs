...
impl<T: Trait> Verifier<T::AccountId> for Module<T> {
	fn verify(_: T::AccountId, _: AppId, message: &Message) -> DispatchResult {
		...
		match message.verification {
			VerificationInput::Basic { block_number, event_index } => Ok(()),
			VerificationInput::LightReceipt { ref receipt, ref proof } => Ok(()),
			VerificationInput::LightTransaction { ref transaction, ref proof } => Ok(()),
